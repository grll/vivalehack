...
class ConversationManager:
    ...
    def __init__(self, agent: Agent, workflow_name: str = "Conversation", default_group_id: str = "1"):
        """
        ...
        try:
            with trace(workflow_name=self.workflow_name, group_id=group_id):
                if not self.group_initialized[group_id]:
                    ...
                    conversation_input = self.last_results[group_id].to_input_list() + [{"role": "user", "content": prompt}]
                ...
                self.conversation_histories[group_id].append({"role": "user", "content": prompt})
                self.conversation_histories[group_id].append({"role": "assistant", "content": result.final_output})
    ...
    def get_all_histories(self) -> Dict[str, List[Dict[str, str]]]:
        ...
        """
        return {group_id: history.copy() for group_id, history in self.conversation_histories.items()}
